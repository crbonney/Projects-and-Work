...
void TestDataLogging_step(void)
{
   ...
   *  Inport: '<Root>/Sensors'
   */
  TestDataLogging_B.x = sensor_inport.HALSensors.HAL_gyro_SI.x;

  /* SignalConversion generated from: '<S2>/Bus Selector4' incorporates:
   *  Inport: '<Root>/Sensors'
   */
  TestDataLogging_B.y = sensor_inport.HALSensors.HAL_gyro_SI.y;

  /* SignalConversion generated from: '<S2>/Bus Selector4' incorporates:
   *  Inport: '<Root>/Sensors'
   */
  TestDataLogging_B.z = sensor_inport.HALSensors.HAL_gyro_SI.z;

  /* SignalConversion generated from: '<S1>/To Workspace' */
  TestDataLogging_B.TmpSignalConversionAtToWorkspac[0] = TestDataLogging_B.x;
  TestDataLogging_B.TmpSignalConversionAtToWorkspac[1] = TestDataLogging_B.y;
  TestDataLogging_B.TmpSignalConversionAtToWorkspac[2] = TestDataLogging_B.z;
  ...
  /* ToWorkspace: '<S1>/To Workspace' */
  {
    ...
    rt_UpdateStructLogVar((StructLogVar *)
                          ...
                          &locTime,
                          &TestDataLogging_B.TmpSignalConversionAtToWorkspac[0]);
  }
  ...
  {                                    /* Sample time: [0.005s, 0.0s] */
    rtExtModeUpload(0, (real_T)TestDataLogging_M->Timing.taskTime0);
  }
  ...
  {                                    /* Sample time: [0.005s, 0.0s] */
    ...
    if (rtmGetStopRequested(TestDataLogging_M)) {
      rtmSetErrorStatus(TestDataLogging_M, "Simulation finished");
    }
  }
...
void TestDataLogging_initialize(void)
{
  ...
  /* External mode info */
  TestDataLogging_M->Sizes.checksums[0] = (542726394U);
  TestDataLogging_M->Sizes.checksums[1] = (417663083U);
  TestDataLogging_M->Sizes.checksums[2] = (4281111958U);
  TestDataLogging_M->Sizes.checksums[3] = (3537687022U);

  {
    static const sysRanDType rtAlwaysEnabled = SUBSYS_RAN_BC_ENABLE;
    static RTWExtModeInfo rt_ExtModeInfo;
    static const sysRanDType *systemRan[3];
    TestDataLogging_M->extModeInfo = (&rt_ExtModeInfo);
    rteiSetSubSystemActiveVectorAddresses(&rt_ExtModeInfo, systemRan);
    systemRan[0] = &rtAlwaysEnabled;
    systemRan[1] = &rtAlwaysEnabled;
    systemRan[2] = &rtAlwaysEnabled;
    rteiSetModelMappingInfoPtr(TestDataLogging_M->extModeInfo,
      &TestDataLogging_M->SpecialInfo.mappingInfo);
    rteiSetChecksumsPtr(TestDataLogging_M->extModeInfo,
                        TestDataLogging_M->Sizes.checksums);
    rteiSetTPtr(TestDataLogging_M->extModeInfo, rtmGetTPtr(TestDataLogging_M));
  }
  ...
  /* data type transition information */
  {
    ...
    (void) memset((char_T *) &dtInfo, 0,
                  sizeof(dtInfo));
    TestDataLogging_M->SpecialInfo.mappingInfo = (&dtInfo);
