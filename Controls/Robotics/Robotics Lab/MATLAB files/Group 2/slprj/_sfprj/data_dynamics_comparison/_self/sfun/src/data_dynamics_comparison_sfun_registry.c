...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_data_dynamics_comparison_process_check_sum_call( int
    nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_data_dynamics_comparison_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_data_dynamics_comparison_process_testpoint_info_call
    ( int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_data_dynamics_comparison_process_testpoint_info_call(nlhs,plhs,nrhs,
       prhs))
    return 1;
  return 0;
}
...
unsigned int sf_process_autoinheritance_call( int nlhs, mxArray * plhs[], int
  nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_data_dynamics_comparison_autoinheritance_info( int nlhs,
  ...
  if (strcmp(machineName, "data_dynamics_comparison") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_data_dynamics_comparison_autoinheritance_info(nlhs,plhs,3,newRhs);
  }
...
{
  extern unsigned int
    sf_data_dynamics_comparison_get_eml_resolved_functions_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  char machineName[128];
  if (nrhs < 3) {
  ...
  if (strcmp(machineName, "data_dynamics_comparison") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_data_dynamics_comparison_get_eml_resolved_functions_info(nlhs,plhs,
      2,newRhs);
  }
...
extern unsigned int sf_data_dynamics_comparison_method_dispatcher(SimStruct *S,
  ...
  method, void *data)
{
  if (!strcmp(machineName,"data_dynamics_comparison")) {
    return(sf_data_dynamics_comparison_method_dispatcher(simstructPtr,
...
extern void data_dynamics_comparison_terminator(void);
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    data_dynamics_comparison_terminator();
    sf_debug_terminate();
  }
...
extern void data_dynamics_comparison_initializer(void);
extern void data_dynamics_comparison_register_exported_symbols(SimStruct* S);
extern void data_dynamics_comparison_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  data_dynamics_comparison_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      data_dynamics_comparison_debug_initialize();
    }

    data_dynamics_comparison_initializer();
  }
